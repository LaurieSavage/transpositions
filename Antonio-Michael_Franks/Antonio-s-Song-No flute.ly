...
\header {
  title = \markup \with-color "tomato" { \realBookTitle }
...
woodwindIntro =  \transpose a g {
  \relative c' {
    ...
    e4. e8~e \breathe r8 e4
    d4 des c gis
    b2~ b8 a4.  %e8
    % d4~ d8 e16 f e4 d
    r2 r4 r8 a8^\markup \with-color "tomato" { "Vocal" }
    \bar "||" \break
  }
}
...
melody =  \transpose a g' {
  \relative c {
    ...
    \repeat volta 2 {
      \bar "[|:-|"
      \mark \default
      c8_\markup { \with-color "tomato" "Solos over Ax4" } \segno d e c~ c e4 c8~
      c8 a4. r4 r8 a8
      c8 d e c~ c e4 a8~
      a2 r4 r8 f8 \break
    ...
    \alternative {
      ...
      {
        r2_\markup \with-color "tomato" { "Vocals re-entry" } c'4 e,8 b'~ \bar"||" \break
      }
    }
...
\book {
  ...
  \score {
    ...
    <<
      ...
      \new Staff \transpose bes c'' \woodwindIntro
    >>
  }
  \score {
    \layout {
